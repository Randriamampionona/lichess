...
export default function Board({
  position, orientation, turn, lastMove, checkSquare, interactive, legalFor, onMove,
}: BoardProps) {
  ...
  const endDrag = (e: React.PointerEvent, apply: boolean) => {
    ...
    pressRef.current = null;
    movedRef.current = false;
    setDrag(null); // React removes the ghost — it can never get orphaned
          ...
          rows.map((dc) => {
            ...
            return (
              <div key={`${dr}-${dc}`} className={cls.join(" ")}>
                ...
                {p && !hidden && (
                  <div className={"piece " + (isW(p) ? "w" : "b")}>{GLYPH[p.toLowerCase() as PieceType]}</div>
                )}
              </div>
            );
          })
        ...
        <div className="promo show">
          <div className={"promo-box " + promotion.color}>
            {(["q", "r", "b", "n"] as PieceType[]).map((t) => (
              <button key={t} onClick={() => pickPromotion(t)} aria-label={`Promote to ${t}`}>
                {GLYPH[t]}
              </button>
            ))}
          </div>
